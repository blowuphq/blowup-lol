...
| dodo_checkout_session_id | text NULL | set at checkout creation (stores Dodo checkout session ID) |
| dodo_payment_id | text NULL UNIQUE | idempotency anchor (F3) (stores Dodo payment ID) |
...
- **Idempotency, three layers:**
  ...
  3. Effect-side: UNIQUE `dodo_payment_id` + strict transition check — even a *different* event referencing the same payment can't credit twice.
...
8. Phase 4 delivered (2026-08-26): public live leaderboard (`/[category]`, `/categories`) with SSE rank updates per §1/§3C. Transport is a Redis Stream per category (`blowup:events:{slug}`, `XADD MAXLEN ~ 500`) — one primitive gives ordered fan-out (XREAD BLOCK) **and** Last-Event-ID replay, and works across serverless instances since the settling webhook and a viewer's SSE connection may run on different ones. Implementation notes recorded against the approved sketch:
   - **Blocking reads isolated.** XREAD BLOCK occupies its connection for the whole block window, so every SSE consumer gets a dedicated reader connection (`openStreamReader()`). Running them on the shared app singleton serialized all publishes/counters behind each 15s block (found in live testing; fixed before review).
   - **Concrete cursors, never `$`.** A fresh connection pins `latestStreamId()` (XREVRANGE COUNT 1) as its start-of-live cursor; `$` is re-resolved to newest-at-each-call by XREAD and would silently skip anything published between two blocked calls.
   - **Absolute-state payloads.** `rank_delta` carries full row state (newRank, score, handle/name/avatar/subscribers/bid total/clicks) — an additive extension of the §3.B10 sketch shape — so replay converges from any retained cursor, and EventSource reconnect pairs Last-Event-ID resume with one fresh `/api/board/:slug` fetch (§3C).
   - **Visitor frames are id-less** (`sseEventOnly`): a frame without an `id:` never touches EventSource's lastEventId, so transient visitor counts can't corrupt the replay cursor. Counter mechanics unchanged from §6 (STRING INCR/DECR, 60s heartbeat TTL refreshed by the hub loop).
   - **§8 connect cap implemented**: fixed-window INCR per IP (60 connects/min), fail-open like every limiter here; settlement publish fails open too (2 attempts) and fires strictly AFTER the PG commit + ZADD in both the fake-bid and webhook paths (§3 ordering invariant).
   - **Board read path** (`features/leaderboard/board.ts`): Redis primary → PG circuit-break flagged `source` (§7.6, amber banner on-page); displayed scores recovered exactly from folded ZSET scores via 4-dp rounding; day-start deltas computed at read time from each creator's first activity today (UTC date_trunc).
...
11. Phase 4.7 delivered (2026-08-27): the local-tooling safety interlock from entry 7 **completed**. That entry wired `assertLocalEnv()` into vitest's `globalSetup` and every `scripts/dev-*` CLI, but three paths that read `.env` were still unguarded — and one of them sits inside a documented routine: `scripts/seed.ts` (INSERTs categories + active seasons; step 1 of the Phase-4.6 verification recipe), `drizzle.config.ts` → `db:push`/`db:migrate` (`push` diffs schema against the live DB and can DROP/ALTER — highest severity of the three), and `npm run dev` (boots the whole app: the SSE hub INCRs visitor counters and `/api/inngest` can ZADD/ZREM). With production DSNs in `.env`, all three resolved production. Closed as follows:
    - **Policy extracted, still defined exactly once.** `assertLocalDsns(databaseUrl, redisUrl)` is now the pure form of the check; `assertLocalEnv()` is a thin `process.env` wrapper over it. Message text and loopback allowlist are unchanged, so the six existing call sites and entry 7's tests are behavior-identical.
    - **A guard must resolve env the way its consumer does.** `next dev` and `dotenv/config` do **not** agree: dotenv reads only `.env` and never clobbers a key already in `process.env`, while Next walks `process.env > .env.$(NODE_ENV).local > .env.local` (skipped when `NODE_ENV=test`) `> .env.$(NODE_ENV) > .env`. A guard that checked the wrong file would either block safe local runs or wave production through, so `scripts/env-preflight.ts` (over a pure, unit-tested `scripts/env-cascade.ts`) replicates BOTH cascades explicitly — `--mode=next` for `dev`, `--mode=dotenv` for the `db:*` scripts — and runs as its own process ahead of the real command (`preflight && command`), so a refusal lands before the consumer is even spawned and nothing connects. Only hostnames are ever printed, never DSN values.
    - **Fail-closed audit.** Unset, unparseable, and exported-but-empty values all abort. One skip is deliberately *not* tolerated: an env file that exists but cannot be read aborts rather than falling through, because silently skipping an unreadable *higher*-precedence file could let a lower-precedence local value win when the true resolution was production — the one skip that is not fail-closed. `dotenv-expand` (`$VAR` interpolation) is intentionally not replicated: the mismatch can only turn a proceed into an abort, never the reverse, since changing a resolved *hostname* requires a `$VAR` inside the URL authority and an unexpanded one leaves a `$` in the hostname, which is not allowlisted.
    - **`scripts/seed.ts`** additionally calls `assertLocalEnv()` as the first statement of `main()` — defense in depth behind the preflight, identical to the `dev-*` CLIs.
    - **`db:generate` verified as needing no live connection** (succeeds both with an unreachable DSN and with no DSN at all — schema → SQL only, no tree change), so it stays ungated on evidence rather than assumption.
    - **`next build` / `next start` are deliberately NOT gated.** Vercel never reads `.env` (it is `.vercelignore`d; production env comes from the Vercel env store), so gating the deploy path would break deploys while protecting nothing.
    Verified end to end: all four gated commands exit 1 in 1–2 ms with the production `.env` in place, and again against a sentinel host (`db.invalid`) so production was never contacted even once during verification; with local DSNs exported all four proceed (`db:seed` → `3 / 3 / 0`, `db:migrate`/`db:push` clean, `npm run dev` boots and `/tech` renders the nine-creator demo board); a temporary `.env.development.local` holding local DSNs unblocks `npm run dev` while `.env` still holds prod — and the rendered rows, which exist only in the local database, prove Next resolved the same file the preflight did rather than the preflight merely agreeing with itself; `npm run build` exits 0 with prod-shaped non-local DSNs and zero `SAFETY ABORT` occurrences; suite **78/78** (68 + 10 new cascade cases in `tests/env-guard.test.ts`).
12. Phase 4.8 delivered (2026-08-28): the Phase-4.5 client-merge gap recorded in `docs/progress.md` **closed**. `applyDelta` — the reducer that folds an SSE `rank_delta` into a board a viewer is already looking at — trusted the ranks it received and then re-sorted by `rank`. A single-entry delta is the *normal* case (one bidder settles, so the event names one creator), which meant every displaced row kept its stale rank: the overtaken #1 and the new #1 both claimed rank 1, the sort tied, the tie fell through to `handle.localeCompare`, and an alphabetically-earlier ex-leader stayed visually on top — two rows numbered "1", no "2" — until a reconnect's fresh fetch healed the board (§3C). Server truth (PG + ZSET) was correct throughout; this was display-only, and only for viewers whose tab was already open.
    - **Ranks are now derived, not received.** After merging the event's entries, the board is sorted by `score` descending and `rank` is reassigned `i + 1`. This is sound precisely *because* `rank_delta` carries absolute state rather than diffs (entry 8): the merged row's score is authoritative, so an event naming one creator is enough to place it correctly against rows it never mentions. The received `newRank` survives only as the first tiebreak when two scores are exactly equal (then handle order), so the tie that used to decide the leader can now only decide between genuinely equal scores.
    - **The one genuine trap: day-start deltas.** `dayDelta` is a *relative* number whose meaning depends on the rank it was measured against (`dayStart = rank + dayDelta`; `null` ⇒ joined today). Carrying it across a resort silently corrupts the "up N / down N today" badge. So the reducer snapshots each row's day-START from the pre-merge board and recomputes `dayDelta = dayStart − newRank` once the final ranks are known; `null` stays `null` instead of decaying into rank arithmetic.
    - **Extracted in order to be testable.** vitest runs in the node environment with no jsdom, so a reducer living inside a `'use client'` screen component could not be unit-tested at all. `applyDelta` moved verbatim into `src/features/leaderboard/apply-delta.ts` in its own commit (`7da72e4`) and the behavior change landed in the next (`f023cd6`), so the diff that changes what viewers see is readable on its own. 10 cases in `tests/apply-delta.test.ts`; 6 of them fail against the pre-fix commit, all 10 pass after — the red run is the evidence that the tests pin the bug and not the implementation.
